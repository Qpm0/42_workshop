...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/09/10 00:09:37 by cmoreanu          #+#    #+#             */
/*   Updated: 2023/10/28 16:06:34 by rpascu           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
*/
static char	*get_expanded_var_line(t_tools *tools, char *line)
{
	char	**words;
	int		i;
	...
	while (words[i])
	{
		...
		{
			words[i] = var_expander_heredoc(tools, words[i]);
			if (!words[i])
				return (NULL);
		}
		i++;
...
*	true.
*/
static bool	check_heredoc_line(t_tools *tools, char **line, t_io_fds *io,
		bool *ret)
{
	if (*line == NULL)
	{
		errmsg_cmd("warning", "heredoc end-of-file: wanted",
	...
	{
		*line = get_expanded_var_line(tools, *line);
		if (!(*line))
		{
			free_ptr(*line);
			*ret = false;
...
bool	fill_heredoc(t_tools *tools, t_io_fds *io, int fd)
{
	...
	while (1)
	{
		set_signals();
		line = readline(">");
		if (!check_heredoc_line(tools, &line, io, &ret))
			break ;
